...
<?php
function widget_letter()
{
    global $CACHE;
    $log_letter = $CACHE->readCache('logletter');
    $log_nav = array_keys($log_letter);
    if (count($log_nav) > 0) : ?>
        <nav>
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                <?php foreach ($log_nav as $k => $v) : ?>
                    <a class="nav-link <?php if ($k == 'A') {echo "active";} ?>" id="nav-<?php echo $v; ?>-tab" data-toggle="tab"
                       href="#nav-<?php echo $v; ?>" role="tab" aria-controls="nav-<?php echo $v; ?>"
                       aria-selected="<?php if ($k == 0) {
                           echo "true";
                       } ?>"><?php echo $v; ?></a>
                <?php endforeach; ?>
            </div>
        ...
        <div class="tab-content" id="nav-tabContent">
            <?php foreach ($log_letter as $k => $v) : ?>
                <div class="tab-pane fade <?php if ($k === 'A') { echo " show active"; } ?>" id="nav-<?php echo $k; ?>" role="tabpanel" aria-labelledby="nav-<?php echo $k; ?>-tab">
                    <ul class="list-inline">
                        <?php foreach ($v as $vs) : ?>
                            <li class="list-inline-item"><a href="<?php echo $vs['url'] ?? ''; ?>"
                                                            title="<?php echo $vs['title']; ?>"><?php echo $vs['title']; ?></a></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            <?php endforeach; ?>
...
function widget_tag($title)
{
    ...
    <div class="widget shadow-theme">
        ...
        <ul class="list-unstyled">
            ...
            <a href="<?php echo Url::tag($value['tagurl']); ?>" title="<?php echo $value['usenum'] + 2; ?> 篇文章"
               class='tags_side'><?php echo $value['tagname']; ?></a></span>
            <?php endforeach; ?>
        </ul>
    </div>
<?php }
    ...
    <div class="navbar-collapse collapse" id="navbarNavDropdown" style="">
        <ul class="navbar-nav">
            <?php foreach ($navi_cache as $value): ?>
                ...
                ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?php echo $value['url']; ?>" <?php echo $newtab; ?> title="<?php echo strtoupper($value['naviname']); ?>"
                       rel="nofollow">
...
{
    extract($neighborLog, EXTR_OVERWRITE); ?>
    <?php if ($prevLog): ?>
...
{
    die('hacker ...');
    extract($comments, EXTR_OVERWRITE);
    if ($commentStacks): ?>
        <a name="comments"></a>
                ...
                <form class="commentform" method="post" name="commentform" action="<?php echo BLOG_URL; ?>index.php?action=addcom" id="commentform">
                    ...
                    <?php if (ROLE == ROLE_VISITOR): ?>
                        <div class="com_info" id="com_info">
                            <input class="form-control com_control com_name" autocomplete="off" type="text" name="comname" maxlength="49"
                                   value="<?php echo $ckname; ?>" size="22"
                                   tabindex="1" placeholder="昵称*" required/>
                            <input class="form-control com_control com_mail" autocomplete="off" type="text" name="commail" maxlength="128"
                                   value="<?php echo $ckmail; ?>" size="22"
                                   tabindex="2" placeholder="邮件地址"/>
                            <input class="form-control com_control com_url" autocomplete="off" type="text" name="comurl" maxlength="128"
