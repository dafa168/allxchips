...
class Cache
{
    ...
    private function mc_options()
    {
        ...
        //设置邮箱和手机号
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user where uid = 1");
        while ($row = $this->db->fetch_array($query)) {
            $options_cache['nickname'] = $row['nickname'] ?? '';
            $options_cache['email'] = $row['email'] ?? '';
            $options_cache['phone'] = $row['phone'] ?? '';
        }
    ...
    private function mc_sort()
    {
        ...
        while ($row = $this->db->fetch_array($query)) {
            ...
            $sortData = array(
                ...
                'seoname' => htmlspecialchars($row['seoname']),
    ...
    private function mc_logletter()
    {

        $sql = "SELECT `gid`,`title`,`letter` FROM " . DB_PREFIX . "blog where type='blog' order by `gid` DESC";
        ...
        $cacheData = serialize($letter);
        $this->cacheWrite($cacheData, 'logletter');
    }
    ...
    {
        $cachefile = EMO_ROOT . '/content/cache/' . $cacheName . '.php';
