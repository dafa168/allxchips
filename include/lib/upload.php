...
//                $str .= "上传文件的大小超过了HTML表单中MAX_FILE_SIZE选项指定的值";
                $str .= "The size of the uploaded file exceeds the value specified by the MAX_FILE_SIZE option in the HTML form";
                break;
            case 1:
//                $str .= "上传的文件超过了php.ini中upload_max_filesize选项限制的值";
                $str .= "The uploaded file exceeds the limit of the \"max_filesize\" option in the configuration file";
                break;
            case -1:
//                $str .= "未允许文件类型";
...
//                $str .= "文件过大,上传的文件不能超过{$this->maxsize}个字节";
                $str .= "The file is too large, the uploaded file cannot exceed {$this->maxsize} bytes";
                break;
            case -3:
//                $str .= "上传失败";
                $str .= "upload failed";
...
//                $str .= "建立存放上传文件目录失败，请重新指定上传目录";
                $str .= "Failed to create a directory to store uploaded files. Please re-specify the upload directory";
                break;
            case -5:
//                $str .= "必须指定上传文件的路径";
                $str .= "The path of the uploaded file must be specified";
                break;
            default:
//                $str .= "未知错误";
                $str .= "unknown error.";
...
//        $this->setOption('fileType', strtolower($aryStr[count($aryStr) - 1]));
        $this->setOption('fileType', strtolower(pathinfo($name, PATHINFO_EXTENSION)));
    ...
    {
        $fileName = date('YmdHis') . "_" . random_int(1000, 9999);
